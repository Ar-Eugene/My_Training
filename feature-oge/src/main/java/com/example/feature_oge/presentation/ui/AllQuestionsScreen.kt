...
import com.example.feature_oge.presentation.viewmodel.AllQuestionsViewModel
import com.example.feature_oge.presentation.viewmodel.QuestionsUiState
...
fun AllQuestionsScreen(
    ...
    taskId: String,
    onBackClick: () -> Unit = {},
    viewModel: AllQuestionsViewModel = hiltViewModel(),
) {
    val backgroundGradientColor = listOf(
        BackgroundGradientGreen, BackgroundGradientBlue
    )

    LaunchedEffect(subjectId, year, taskId) {
        viewModel.loadQuestions(subjectId, year, taskId)
    }
    ...
    when (val state = uiState) {
        ...
        is QuestionsUiState.Error -> {
            Column(
                ...
                TopIconButtonAndText(onClick = onBackClick, title = "Все вопросы")
                Text(
                    text = state.message,
        ...
        is QuestionsUiState.Success -> {
            AllQuestionsContent(
                subjectName = viewModel.subjectName.orEmpty(),
                year = year,
...
@Composable
private fun AllQuestionsContent(
    subjectName: String,
    year: Int,
    allQuestions: List<QuestionContext>,
    onBackClick: () -> Unit,
) {
    val backgroundGradientColor = listOf(
        BackgroundGradientGreen, BackgroundGradientBlue
    ...
    Column(
        ...
        TopIconButtonAndText(
            ...
            title = "Все вопросы: $subjectName $year"
        )
